...
//!
//! These drive the real `tiles` binary and stand a second machine up in the
//! same way the real thing works: a node with its own config directory and its
//! own socket, reached by running `tiles agent` over "ssh". `TILES_SSH` points
...
const TILES: &str = env!("CARGO_BIN_EXE_tiles");
...
impl World {
    fn new(name: &str) -> Self {
        ...
        let dir = std::env::temp_dir().join(format!("tiles-t-{name}-{}", std::process::id()));
...
exec env TILES_CONFIG_DIR="{dir}/$host" "{tiles}" --socket "{dir}/$host.sock" agent
"#,
    ...
    /// Run a tiles command as `machine`, with its own config and socket.
    fn run(&self, machine: &str, args: &[&str]) -> Output {
        Command::new(TILES)
            .arg("--socket")
            .arg(self.socket(machine))
            .args(args)
            .env("TILES_CONFIG_DIR", self.dir.join(machine))
            .env("TILES_SSH", self.ssh_stub())
            // Keep the noise down; failures print stderr below.
            .env("TILES_LOG", "tiles=warn")
            .output()
            .expect("running tiles")
    }
...
/// Using a machine is enough to have it remembered; `tiles add` is only for
...
/// what tiles hands to ssh is a plain destination plus `tiles agent`.
#[test]
fn ssh_is_invoked_with_the_destination_and_nothing_surprising() {
    ...
    assert!(args.contains(&"tiles"), "does not run tiles: {args:?}");
...
/// test: with no `TILES_SSH` at all, tiles must still try to run `ssh`.
#[test]
fn the_real_ssh_is_the_default() {
    let dir = std::env::temp_dir().join(format!("tiles-t-default-{}", std::process::id()));
    std::fs::create_dir_all(&dir).unwrap();
    let out = Command::new(TILES)
        .arg("--socket")
        .arg(dir.join("laptop.sock"))
        .args(["ls", "definitely-not-a-real-host.invalid"])
        .env("TILES_CONFIG_DIR", &dir)
        .env("TILES_LOG", "tiles=warn")
        .env_remove("TILES_SSH")
        .output()
        .expect("running tiles");
